...
typedef enum
{
	RX_STATE_INIT=0, //Application's state machine's initial state.

    /* TODO: Define states used by the application state machine. */
    //In any of the below states, the machine is waiting for that particular byte
    // E.G. Start state would mean it is waiting for the start byte 
    RX_STATE_START,       // USART transmit state
    RX_STATE_TYPE,        // USART receive state
    RX_STATE_D1,        // USART receive state
    RX_STATE_D2,        // USART receive state
            
    //This state is entered when a stop bit is received
    RX_STATE_INTERPRET        // USART processing state (async call to other functions/Queues?)
