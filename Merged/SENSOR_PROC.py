...
def processMsgFromCoordinator(type, MESSAGE):
   ...
   elif type == TYPEC_LR_SENSOR_TO_FR:
        print("The coordinator is trying to tell the FR that it is on top of a token, and there are {0} token(s) left.".format(int(hex(ord(MESSAGE[8])),0))) 
        sock.sendto(MESSAGE, (FOLLOWER_IP, FOLLOWER_PORT))

   elif type == TYPEC_GO:
        print("The coordinator has instructed the FR to GO")
        sock.sendto(MESSAGE, (FOLLOWER_IP, FOLLOWER_PORT))        
        

   elif type == TYPEC_TOKEN_FOUND_ACK:
        print("The coordinator has ACKnowledged that the FR has said it picked up a token. The token count is now {0}".format(int(hex(ord(MESSAGE[8])),0)))
        sock.sendto(MESSAGE, (FOLLOWER_IP, FOLLOWER_PORT))        
        
   elif type == TYPEC_ACK_SENSOR_DATA:
        print("The coordinator has ACKnowleged a message from the sensor with index value: {0}".format(int(hex(ord(MESSAGE[6])),0)))
        processMsgToSensor(MESSAGE)        
...
def processMsgFromSensor(type, MESSAGE):
    ...
    print("Message received from SENSOR: {0}".format(type))
    print(" ".join(hex(ord(n)) for n in MESSAGE))
    ...
    if type == TYPE_SENSOR_APPENDMAP:
        ...
        #print('End of sensor update') 
         ...
         print("The sensor has ACKnowleged a message from the coordinator")
         processMsgToCoordinator(MESSAGE)        

def processMsgFromDaniel(type, num):
    ...
    if type == TYPEC_GO:
        print("GO")
    elif type == TYPEC_STOP:
        print("STOP")
    elif type == TYPEC_LR_SENSOR_TO_FR:
        print("FOUND TOKEN")
    elif type == TYPE_HISTORY:
        print("ALTERING TURNS")
    elif type == TYPE_FR_ACK:
        print("ALTERING STOP ON TOKEN")
    elif type == TYPE_MS4SET:
        print("Demo #{0}".format(num))
    else:
        return
    ...
    MSG = '{0}{1}xddddd{2}{3}'.format(MESSAGE_START_BYTE, type, put, MESSAGE_STOP_BYTE)
    sock.sendto(MSG, (FOLLOWER_IP, FOLLOWER_PORT))
