...
void receiveDataFromISR() {
    ...
    while (uxQueueMessagesWaitingFromISR(sendData.transmitQ_FR) != 0){
        while (xQueueReceiveFromISR(sendData.transmitQ_FR, (void *) &readChar, &xTaskWokenByReceive) ) 
        {
...
void sendDataToTransmitQ(char* message) {
    ...
    if (*message == '~') {
        ...
        // SEND TO UART QUEUE
        if (sendData.transmitQ_FR != 0) {

            //for (k = message; *k; ++k) {
            // Add message to queue, character by character
            for (iter = 0; iter < MSG_LENGTH; iter++) {

                if( xQueueSend( sendData.transmitQ_FR, (void*) k, portMAX_DELAY) != pdPASS )
                {
...
{    
    ...
    if (xQueueReceive(sendData.sendQ_FR, &read, portMAX_DELAY))
    {
...
uint8_t removeSendQueueData() {
    ...
    if (xQueueReceive(sendData.sendQ_FR, &readdata, portMAX_DELAY))
    {
        return 0x1;
    } 
...
void putMsgOnSendQueue(char* data) {
    ...
    if (sendData.sendQ_FR != 0) {
        ...
        if( xQueueSend( sendData.sendQ_FR, (void*) data, portMAX_DELAY) != pdPASS )
        {
...
void SEND_Initialize ( void )
{
    ...
    sendData.transmitQ_FR = xQueueCreate( 1000, sizeof(char) );
    if( sendData.transmitQ_FR == 0 ) {
    ...
    sendData.sendQ_FR = xQueueCreate(250, MSG_LENGTH+1);
    if (sendData.sendQ_FR == 0) {
        dbgOutputVal(SEND_QUEUE_FAIL);
        stopAll();
    }
...
{    
    ...
    if (sendData.transmitQ_FR != 0) {
        if( xQueueSend( sendData.transmitQ_FR, (void*) test, portMAX_DELAY) != pdPASS )
        {
...
void sendValFromISR(unsigned int* message)
{
    ...
    if (xQueueSendFromISR( sendData.transmitQ_FR,
                            (void*) message,
...
void receiveDataFromMsgQ() {
    ...
    if (uxQueueMessagesWaiting(sendData.transmitQ_FR) != 0){
        //stopAll();
        //writeString("Asking Queue...");
        if (xQueueReceive(sendData.transmitQ_FR, &readdata, portMAX_DELAY))
        {
            dbgOutputVal(SEND_RECEIVEFROMQ);
            newData = 1;
        } 
