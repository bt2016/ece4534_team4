...
void motorSendToProcessQ() {
    ...
    data[1] = TYPE_FR_ENCODER;      // Type byte
...
uint8_t removeMotorQueueData() {
    ...
    if (xQueueReceive(motorData.MotorQ_FR, &readdata, portMAX_DELAY))
    {
        return 0x1;
    } 
...
void putDataOnMotorQ(char* data) {
    if (motorData.MotorQ_FR != 0) {
        
        // Check for full queue. If no spaces available, call to remove oldest data.
        if (uxQueueSpacesAvailable( motorData.MotorQ_FR ) == 0) {
        ...
        if( xQueueSend( motorData.MotorQ_FR, (void*) data, portMAX_DELAY) != pdPASS )
        {
...
void processMotorInstruction(char* data) {
...
void receiveFromMotorQ() {
    ...
    while (uxQueueMessagesWaiting(motorData.MotorQ_FR) != 0){
    
        if (xQueueReceive(motorData.MotorQ_FR, &readdata, portMAX_DELAY))
        {
            //dbgOutputVal(MOTOR_RECEIVEFROMQ);
            processMotorInstruction(readdata);
        } 
    }
}
...
void MOTOR_Initialize ( void )
{
    ...
    motorData.MotorQ_FR = xQueueCreate( 250, MSG_LENGTH+1 ); 
    if( motorData.MotorQ_FR == 0 ) {
    ...
    //Create a queue capable of holding 250 system messages
    motorData.actuatorQ_FR = xQueueCreate( 250, MSG_LENGTH+1 ); 
    if( motorData.actuatorQ_FR == 0 ) {
        dbgOutputVal(MOTOR_QUEUE_FAIL);
        stopAll();
    }
